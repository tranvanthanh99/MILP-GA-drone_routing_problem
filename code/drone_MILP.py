...
def create_data_model():
    ...
    data['max_payload'] = 8
    data['max_energy'] = 60
    data['gamma0'] = 0.3    # energy for takeoff and landing
    data['gamma'] = 0       # energy for takeoff and landing with w package
    data['p0'] = 0.2        # energy to fly one distance unit
    data['p'] = 0           # energy to fly one distance unit with 1 pakage
...
def MILP(gen_data=False, quantity=0):
    ...
    for i in range(2, data['num_locations']):
        demand = float(data['locations'][i]['demand'])
        # demandConstraint = solver.Constraint(data['locations'][i]['demand'], data['locations'][i]['demand'])
        demandConstraint = solver.Constraint(demand, demand)
        for j in range(data['num_locations']):
            if i != j:
